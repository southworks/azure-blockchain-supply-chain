...
namespace SupplyChain.Stock
{
	public class UpdateItemDescription
	{
		...
		public UpdateItemDescription(IUserProfileService userProfileService)
		{
			_userProfileService = userProfileService;
		}

		[FunctionName("UpdateItemDescription")]
			...
			ILogger log)
		{
			try
			{
				...
				var updateItemDescription = JsonConvert.DeserializeObject<UpdateItemDescriptionFunction>(requestBody);

				await stockService.UpdateItemDescriptionRequestAsync(updateItemDescription);

				log.LogInformation($"Description of item {updateItemDescription.Id} updated to: {updateItemDescription.Description}");

				return new OkResult();
			}
			catch (Exception e)
			{
				log.LogInformation($"The following Exception was raised in UpdateItemDescription: {e.Message}");
