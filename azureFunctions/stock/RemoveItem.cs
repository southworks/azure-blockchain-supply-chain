...
namespace SupplyChain.Stock
{
	public class RemoveItem
	{
		...
		public RemoveItem(IUserProfileService userProfileService)
		{
			...
			ILogger log)
		{
			try
			{
				...
				var removeItem = JsonConvert.DeserializeObject<RemoveItemFunction>(requestBody);

				await stockService.RemoveItemRequestAsync(removeItem);

				log.LogInformation($"Item with id {removeItem.Id} removed.");

				return new OkResult();
			}
			catch (Exception e)
			{
				log.LogInformation($"The following Exception was raised in RemoveItem: {e.Message}");
