...
namespace SupplyChain.Retailer
{
	public class DeliverOrder
	{
		...
		public DeliverOrder(IUserProfileService userProfileService)
		{
			...
			ILogger log)
		{
			try
			{
				...
				var deliverOrder = JsonConvert.DeserializeObject<DeliverOrderFunction>(requestBody);

				await retailerService.DeliverOrderRequestAsync(deliverOrder);

				log.LogInformation($"Order {deliverOrder.OrderId} delivered.");

				return new OkResult();
			}
			catch (Exception e)
			{
				log.LogInformation($"The following Exception was raised in DeliverOrder: {e.Message}");
